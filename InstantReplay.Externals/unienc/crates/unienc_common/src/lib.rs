...
pub trait EncodingSystem {
    ...
    type GraphicsEventIssuerType: GraphicsEventIssuer;

    fn new(
        video_options: &Self::VideoEncoderOptionsType,
        audio_options: &Self::AudioEncoderOptionsType,
    ) -> Self;
    fn new_video_encoder(&self, issuer: Self::GraphicsEventIssuerType) -> Result<Self::VideoEncoderType>;
...
pub trait GraphicsEventIssuer: Send + 'static {
    fn issue_graphics_event(&self, callback: impl FnOnce() + Send, event_id: i32);
}
